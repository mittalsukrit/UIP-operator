...
class MOEAD_LWS(GeneticAlgorithm):
    ...
    def _next(self):
        ...
        # print(F_norm.shape)
        F_inverted = 1 - F_norm #DEXTER - inverted
        theta = np.array([[calc_angle(f,w) for w in self.ref_dirs] for f in F_inverted]) #DEXTER - inverted
        ...
        S = []
        nr = int(round(0.1*self.n_neighbors,0)) #from LWS paper
        # print(nr, self.n_neighbors) #just checking
        if nr==0:
            nr = 1 # setting the minimum possible value
        selection_count = np.zeros(len(F_norm))
        ...
        # for i in np.random.permutation(self.pop_size):
            ...
            if self.problem.n_constr>0:
                ...
                # S.append(order[index])
                if len(order)>0:
                    self.pop[i] = JointS[order[0]]
            # for attempt in range(self.pop_size):
            #     if selection_count[order[index]]<nr:
            #         S.append(order[index])
            #         selection_count[order[index]] += 1
            #         break
            #     else:
            #         index += 1
        
        # structure the final surviving population
        # self.pop = JointS[S]
        self.assoc_index = [i for i in range(self.pop_size)]
        self.all_F = (self.pop.get("F"))
        self.all_CV = (self.pop.get("CV"))
        # print(np.max(self.pop.get("F"),axis=0))
        # print(max(self.pop.get("CV")))
