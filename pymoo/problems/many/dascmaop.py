...
class DASCMAOP(Problem):

    def __init__(self, n_var, n_obj, difficulty, k=None, l=None, **kwargs):
    ...
    def validate(self, l, k, n_obj):
        if n_obj < 2:
            raise ValueError('DASCMAOP problems must have two or more objectives.')
        if not k % (n_obj - 1) == 0:
    ...
    def constraints(self, X, F): # do it individual by individual 
        [eta, zeta, gamma] = self.difficulty
        M = self.n_obj
        a = 20
        b = eta
        c = 0.5
        d = c + math.pow(10,-2*zeta)
        r = 0.5*gamma
        sRange = [2*(i+1) for i in range(M)]
        con = np.zeros((X.shape[0], 2*M+1))
        for z in range(X.shape[0]):
            for i in range(M-1):
                if eta < 1e-06:
                    break
                if i%2==0:
                    con[z,i] = math.sin(a * math.pi * X[z,i]) - b
                else:
                    con[z,i] = math.cos(a * math.pi * X[z,i]) - b
            if zeta > 1e-06:
                con[:,M-1] = ((d - X[:,M - 1])*(X[:,M - 1] - c))
            for i in range(M):
                sum_c = 0
                for j in range(M):
                    sum_c += F[z,j]*F[z,j]
                sum_c = sum_c - F[z,i] * F[z,i] + (F[z,i] - 1) * (F[z,i] - 1) - r * r
                con[:,M+i] = sum_c
                
            sum_c = 0
            for i in range(M):
                sum_c = sum_c + math.pow(F[z,i]-math.sqrt(1/M),2) - r * r
            con[:,M+M] = sum_c
...
class DASCMAOP1(DASCMAOP):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        y = x / self.xu
        y = DASCMAOP1.t1(y, self.n_var, self.k)
        y = DASCMAOP1.t2(y, self.n_var, self.k)
        y = DASCMAOP1.t3(y, self.n_var)
        y = DASCMAOP1.t4(y, self.n_obj, self.n_var, self.k)
        ...
        out["G"] = self.constraints(x, out["F"])

    def _rand_optimal_position(self, n):
        return np.power(np.random.random((n, self.k)), 50.0)


class DASCMAOP2(DASCMAOP):

    def validate(self, l, k, n_obj):
        super().validate(l, k, n_obj)
        validate_DASCMAOP2_DASCMAOP3(l)
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        y = x / self.xu
        y = DASCMAOP1.t1(y, self.n_var, self.k)
        y = DASCMAOP2.t2(y, self.n_var, self.k)
        y = DASCMAOP2.t3(y, self.n_obj, self.n_var, self.k)
        y = self._post(y, self.A)
...
class DASCMAOP3(DASCMAOP):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        # from optproblems.DASCMAOP import DASCMAOP3 as DASCMAOP3opt
        # out["F"] =  np.array([DASCMAOP3opt(self.n_obj, self.n_var, self.k).objective_function(_x) for _x in x])


class DASCMAOP4(DASCMAOP):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        y = x / self.xu
        y = DASCMAOP4.t1(y)
        y = DASCMAOP4.t2(y, self.n_obj, self.k)
        y = self._post(y, self.A)
...
class DASCMAOP6(DASCMAOP):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        y = x / self.xu
        y = DASCMAOP1.t1(y, self.n_var, self.k)
        y = DASCMAOP6.t2(y, self.n_obj, self.n_var, self.k)
        y = self._post(y, self.A)
...
class DASCMAOP8(DASCMAOP):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        y = x / self.xu
        y[:, self.k:self.n_var] = DASCMAOP8.t1(y, self.n_var, self.k)
        y = DASCMAOP1.t1(y, self.n_var, self.k)
        y = DASCMAOP4.t2(y, self.n_obj, self.k)
        y = self._post(y, self.A)
...
class DASCMAOP9(DASCMAOP):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        y = x / self.xu
        y[:, :self.n_var - 1] = DASCMAOP9.t1(y, self.n_var)
        y = DASCMAOP9.t2(y, self.n_var, self.k)
        y = DASCMAOP9.t3(y, self.n_obj, self.n_var, self.k)
...
def validate_DASCMAOP2_DASCMAOP3(l):
    if not l % 2 == 0:
        raise ValueError('In DASCMAOP2/DASCMAOP3 the distance-related parameter (l) must be divisible by 2.')
