...
class DTLZ(Problem):
    ...
    #DEXTER
    def g3(self, X_, X_M):
        return anp.sum(anp.square(X_M - X_[:,[0]]), axis=1)
    ...
    #DEXTER
    def constraints(self, F, g): #do it individual by individual
        con = np.zeros([F.shape[0], 1])
        for i in range(F.shape[0]):
            if g[i] > 0.005:
                if max(F[i])>0.8 or min(F[i])<0.2:
                    con[i, 0] = g[i]
        return con
...
class IC_DTLZ2(DTLZ):
    def __init__(self, n_var=10, n_obj=3, n_constr=1, **kwargs):
        super().__init__(n_var, n_obj, **kwargs)
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        g = self.g3(X_, X_M)
        out["F"] = self.obj_func(X_, g, alpha=1)
        out["G"] = self.constraints(out["F"], g)
