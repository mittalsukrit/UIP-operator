...
class WFG1(WFG):
    ...
    def t3(x, n):
        ...
        # x[:, :n] = _transformation_bias_poly(x[:, :n], 0.2) #DEXTER - from Haitham's B-NSGA-III paper
        return x
...
class WFG4(WFG):
    ...
    def t1(x):
        ...
        # return _transformation_shift_multi_modal(x, 70.0, 5.0, 0.35) #DEXTER - for special case in multi-modal
...
class WFG7(WFG):
    ...
    def t1(x, k):
        for i in range(k):
            ...
            x[:, i] = _transformation_param_dependent(x[:, i], aux, A=0.98 / 49.98, B=0.02, C=50.0)
            # x[:, i] = _transformation_param_dependent(x[:, i], aux, A=0.98 / 49.98, B=0.02, C=100.0) #DEXTER - increased bias special case
        return x
