...
class ZDT1(ZDT):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        f1 = x[:, 0]
        g = 1 + 9.0 / (self.n_var - 1) * anp.sum((x[:, 1:]-0.5)**2, axis=1)
...
class ZDT2(ZDT):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        f1 = x[:, 0]
        c = anp.sum((x[:, 1:]-0.5)**2, axis=1) #changed to 0.5 - DEXTER
...
class ZDT4(ZDT):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        for i in range(1, self.n_var):
            ...
            g += (x[:, i]-0.5) * (x[:, i]-0.5) - 10.0 * anp.cos(4.0 * anp.pi * ((x[:, i]-0.5)**2))
        h = 1.0 - anp.sqrt(f1 / g)
        f2 = g * h
...
class ZDT6(ZDT):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        g = 1 + 9.0 * anp.power(anp.sum((x[:, 1:]-0.5)**2, axis=1) / (self.n_var - 1.0), 0.25) #0.25 - DEXTER
